...
class Action:
    def __init__(self, agent, name, stat):
        self.agent = agent
        self.name = name
        self.stat = stat
...
class SUDPostaControllerV2: 
    ...
    def input_commands_callback(self, data):
        ...
        valid_command, response, command, agent, startstop, p1, p2 = self.parse_command(self.latest_command_string)

        if (valid_command):
            
            self.action_list.append(Action(command, agent, startstop))
    ...
    def parse_command(self,command_string): 
        if(len(command_string)>0):
            ...
            if(values[0] in PostaCommands._member_names_ and len(values)>=3):
                ...
                return True,'',command, agent, startstop, parameter1, parameter2
    ...
    def ControllerStart(self):
        ...
        while not rospy.is_shutdown():
            ...
            if (len(self.commands_queue) > 0):
                ...
                valid_command, response, command, agent, startstop, p1, p2 = self.parse_command(command_string)
